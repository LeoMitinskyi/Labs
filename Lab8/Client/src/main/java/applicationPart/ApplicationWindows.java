...
public class ApplicationWindows extends Application {
    ...
    public void start(Stage authStage) throws Exception{
        ...
        authStage.setScene(new Scene(root, 1000, 600));
        ...
        authorizationWindowController.englishMenuItem.setOnAction(event -> {
            Locale.setDefault(new Locale("en", "EN"));
            Messages.changeBundle();
            Messages.changeAuthorizationWindow(authorizationWindowController);
        });
        authorizationWindowController.finnishMenuItem.setOnAction(event -> {
            Locale.setDefault(new Locale("fl"));
            Messages.changeBundle();
            Messages.changeAuthorizationWindow(authorizationWindowController);
        });
        authorizationWindowController.greekMenuItem.setOnAction(event -> {
    ...
    public void mapDrawing(String dataToDraw) {
        ...
        /*mapController.map.setOnMouseDragged(event -> {
            ...
            xOffset = event.getScreenX();
            yOffset = event.getScreenY();
            for (ImageView circle : circles.values()) {
                AnchorPane.setBottomAnchor(circle, AnchorPane.getBottomAnchor(circle) - y);
                AnchorPane.setLeftAnchor(circle, AnchorPane.getLeftAnchor(circle) + x);
            }
            MenuButton finalAvailableTickets = availableTickets;
            if (finalAvailableTickets != null) {
                AnchorPane.setBottomAnchor(finalAvailableTickets, AnchorPane.getBottomAnchor(finalAvailableTickets) - y);
                AnchorPane.setLeftAnchor(finalAvailableTickets, AnchorPane.getLeftAnchor(finalAvailableTickets) + x);
            }
        });*/
        mapController.map.setOnMouseClicked(event -> {
            ...
            for (Integer id : circles.keySet()) {
                ...
                if (AnchorPane.getBottomAnchor(circle) <= 600 - event.getSceneY() + 50 && AnchorPane.getBottomAnchor(circle) >= 600 - event.getSceneY() - 50 &&
            ...
            if (!menuItems.isEmpty()) {
                ...
                AnchorPane.setBottomAnchor(availableTickets, 600 - event.getSceneY());
    ...
    public void setInfoPanel() throws IOException {
        ...
        infoStage.setScene(new Scene(root, 400, 600));
        setInfoPanelButtons();
